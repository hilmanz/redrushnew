...
class MOPClient{
	...
	function checkReferral($sessid){
		...
		*/
		...
		$debug->addLog('CheckReferral',$this->getActiveUserName(),$req_url,
						"SendAction2",$data['CheckReferral']['SessionID'],$data['CheckReferral']['Result']);
	...
	function track($sessId,$PageRef,$ActivityName,$ActivityValue,$CPMOO,$user){
		...
		*/
		...
		}else $this->setSession($sessId);
		$debug->addLog($CPMOO['Phase']." - ".$ActivityName."-".$ActivityValue,$this->getActiveUserName(),$req_url,
						"SendAction2",$_SESSION['mop_sess_id'],$data['SendAction']['Result']);
		$track_end = time();
	...
	function GetProfile2($option,$sessid){
		...
		*/
		...
		$debug->addLog('GetProfile2',$this->getActiveUserName(),$req_url,
						"GetProfile2",$data['GetProfile']['SessionID'],$data['GetProfile']['Result']);
	...
	function XMLToArrayFlat($xml, &$return, $path='', $root=false)
{
	...
	function getUserInfo(){
		...
		$rs = $this->fetch("SELECT * FROM kana_member WHERE register_id='".$id."' LIMIT 1");
