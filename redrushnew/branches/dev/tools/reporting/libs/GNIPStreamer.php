...
class GnipPowerTrack_Consume {
	...
	protected $gnip_powertrack_location = 'https://kanadigital-powertrack.gnip.com/data_collectors/1/track.json';
	protected $gnip_powertrack_rules = 'https://kanadigital-powertrack.gnip.com/data_collectors/1/rules.json';
	protected $gnip_powertrack_rules_xml = 'https://kanadigital-powertrack.gnip.com/data_collectors/1/rules.xml';
	// Set this to something identifiable in case Gnip needs to troubleshoot
	protected $user_agent = 'KANADIGITAL';
	
	// path to log data
	protected $log_location = 'stream.log';
	...
	protected $volume = 1000;
	protected $n_feed = 0;
	protected $n_current_rules = 0;
	protected $keywords;
	protected $keyword_cap = 1000;
	protected $type; // 0->enterprise(default),  1-> power track
	protected $terminated = false;
	protected $disconnect = false;
	public function __construct($username, $password, $volume = 1000, $keyword_cap=1000,$type=1,
								$source_uri="https://kanadigital-powertrack.gnip.com/data_collectors/1/track.json",
								$source_rule="https://kanadigital-powertrack.gnip.com/data_collectors/1/rules.json") {
		$this->username = $username;
		$this->password = $password;
		$this->volume = $volume;
		$this->gnip_powertrack_location = $source_uri;
		$this->gnip_powertrack_rules = $source_rule;
		$this->type = $type;
		$this->keywords = array();
		$this->keyword_cap = $keyword_cap;
	...
	private function get_rules(){
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $this->gnip_powertrack_rules);
		curl_setopt($ch, CURLOPT_USERAGENT, $this->user_agent);
		curl_setopt($ch, CURLOPT_USERPWD, $this->username . ":" . $this->password);  
		curl_setopt($ch, CURLOPT_ENCODING, "");
		curl_setopt($ch, CURLOPT_NOSIGNAL, true);
	...
	public function consume() {
		...
		if($this->n_current_rules>0){
			$this->volume = $this->n_current_rules * $this->keyword_cap;
			$this->write_log('info', 'Volume cap : '.$this->n_current_rules.'x10000 = '.$this->volume);
			...
			//no keyword yet
			$this->write_log('error', 'No keyword available in the feed. Sleep for 5s');
			sleep(5);
		}
		...
		if($this->terminated){
			$this->write_log('notice', 'The job is terminated gracefully :) ');
		}else{
		    $this->consume();
		}
	}
	...
	public function log($message='') {
		...
		if (($this->last_log_time+60) <= $now) {
			...
			foreach($this->keywords as $k_id=>$k_count){
				$this->write_log('info','Keyword '.$k_id.' -> '.$k_count);
			}
	...
	public function enqueue($message) {
		...
		print $o->gnip->matching_rules[0]->value." - ".$o->gnip->matching_rules[0]->tag."\n\n";
		if($this->keywords[$o->gnip->matching_rules[0]->tag]==null){
			$this->keywords[$o->gnip->matching_rules[0]->tag] = 0;
		}
		$this->keywords[$o->gnip->matching_rules[0]->tag]+=1;
		if($this->keywords[$o->gnip->matching_rules[0]->tag]>=$this->keyword_cap){
			$this->remove_keyword($o->gnip->matching_rules[0]->tag,$o->gnip->matching_rules[0]->value);
			sleep(3); //sleep so the keyword_updater bot have time to process the keyword removal from gnip system.
		
			$this->write_log("info","Assuming keyword has been removed, so we disconnect the app (sleep for 3s)");
			$this->disconnect = true;
		}else{
			$this->n_feed++;
		}
	...
	private function remove_keyword($id,$keyword){
		global $conn;
		$this->write_log("info","Removing keywords --> ".$id." | ".$keyword);
		//$updater = new GNIPKeywordUpdater($this->username,$this->password,$this->gnip_powertrack_rules_xml);
		//$updater->deleteKeyword(array("id"=>$id,"txt"=>$keyword));
		//$updater->execute();
		$sql = "UPDATE smac.tbl_keyword_power SET n_status=2 WHERE keyword_id = ".trim($id)."";
